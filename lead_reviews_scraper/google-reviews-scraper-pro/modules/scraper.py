...
REVIEW_WORDS = {
    ...
    "جائزے", "تبصرے", "تجزیہ", "رائے", "درجہ بندی"
}
...
class GoogleReviewsScraper:
    ...
    def __init__(self, config: Dict[str, Any],
        ...
        self.place_id = None  # Store the discovered place_id
    ...
    def is_reviews_tab(self, tab: WebElement) -> bool:
        ...
        """
        try:
            # Strategy 1: Data attribute detection (only trust explicit "reviews" value, not numeric index)
    ...
    def click_reviews_tab(self, driver: Chrome):
        ...
        tab_selectors = [
            # Direct tab selectors (candidates only — is_reviews_tab() validates each)
            '[role="tab"][data-tab-index]',  # Any tab with index
            '[data-tab-index]',              # Broader index-bearing elements
    ...
    def verify_reviews_tab_clicked(self, driver: Chrome) -> bool:
        ...
        """
        try:
            # URL check first - most reliable
            if "review" in driver.current_url.lower():
                return True

            # Review-specific elements (these do NOT appear on Menu/Overview tabs)
            review_specific_selectors = [
                'div[data-review-id]',                    # actual review cards
                'button[aria-label*="Sort" i]',           # sort button only on reviews
                'span[role="img"][aria-label*="star" i]', # star ratings in reviews
                'div.m6QErb div.jftiEf',                  # review item container
                '.HlvSq',                                 # reviewer name class
            ]
    ...
    def scrape(self):
        ...
        try:
            ...
            self.place_id = self.review_db.upsert_place(
                place_id, place_name, url, resolved_url, lat_f, lng_f
            )
            session_id = self.review_db.start_session(self.place_id, sort_by)
            log.info(f"Registered place: {self.place_id} ({place_name})")

            # Load seen IDs from DB (empty for full mode to re-process everything)
            if self.scrape_mode == "full":
                seen = set()
            else:
                seen = self.review_db.get_review_ids(self.place_id)
            ...
            reviews = self.review_db.get_reviews(self.place_id) if self.place_id else []
            if reviews:
                legacy_docs = {
                ...
                try:
                    runner.run(legacy_docs, self.place_id, seen=seen)
                finally:
                    runner.close()
